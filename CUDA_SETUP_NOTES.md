...
- [x] CUDA 側のカメラ設定を CPU 版に近づける。
- [x] defocus blur を CUDA 側で扱う。
- [x] ランダムな多数の球を CUDA 側で扱えるようにする。
- [x] コピー元の最終シーンに近い構成で CPU 版と CUDA 版を比較する。
...
## 16. CUDA 側のカメラ設定を CPU 版に近づける

これまでの CUDA 側カメラは、原点から `z = -1` 方向を見る固定の簡易カメラだった。
CPU 版の `camera` では、`lookfrom` / `lookat` / `vup` / `vfov` / `focus_dist` からカメラ座標系を作り、`pixel00_loc` と `pixel_delta_u/v` を使ってレイを生成している。

そこで CUDA 側にも `cuda_camera` を追加し、CPU 版と同じ考え方でレイを作るようにした。
...
CUDA 側の camera ray は、CPU 版と同じようにピクセル中心 `pixel00_loc` からランダムに少しずらした位置へ飛ばす。
...
CUDA path traced spheres render:
  ...
  focus_dist: 1
  total primary samples: 448000
  time: 0.283674 seconds
  primary samples/sec: 1.57928e+06
...
CPU render:
  ...
  max_depth: 10
  total primary samples: 448000
  Render time: 1.12203 seconds
  Primary samples/sec: 399276
```

この段階で、CUDA 側でも固定カメラではなく、CPU 版と同じカメラパラメータからレイを生成できるようになった。

## 17. defocus blur を CUDA 側で扱う

CPU 版のカメラでは、`defocus_angle` が 0 より大きい場合、レイの始点をカメラ中心ではなく defocus disk 上のランダムな点にする。
これにより、焦点距離から外れた場所がぼける。

CUDA 側でも同じ考え方を使えるように、`cuda_camera` に defocus disk の情報を追加した。
...
また、GPU 側でレンズ上のランダム点を選べるように `random_in_unit_disk()` を追加した。
...
CUDA path traced spheres render:
  ...
  defocus_angle: 2
  total primary samples: 448000
  time: 0.268302 seconds
  primary samples/sec: 1.66976e+06
...
CPU render:
  ...
  max_depth: 10
  total primary samples: 448000
  Render time: 1.41496 seconds
  Primary samples/sec: 316616
```

この小さな比較シーンでは焦点距離が中央付近にあるため、見た目のぼけは控えめ。
ただし、CUDA 側でも CPU 版と同じように defocus disk から ray origin をサンプリングできるようになった。

## 18. ランダムな多数の球を CUDA 側で扱えるようにする

これまでは、地面と 3 個の大きな球だけを固定で置いていた。
コピー元の最終シーンに近づけるため、CUDA 側でも多数の小さな球を扱えるようにした。

今回の方針:

- CPU 側で `std::vector<cuda_sphere>` を作る。
- 固定の 4 球に加えて、小さい球をグリッド状に候補生成する。
- 疑似乱数で位置、半径、マテリアル、色、fuzz を決める。
- 生成した球配列を `cudaMemcpyHostToDevice` で GPU へ送る。
- CUDA kernel では今まで通り全球を走査し、一番近い hit を採用する。

CPU 版との比較用に、`main.cpp` 側にも同じ疑似乱数と同じ生成ルールを置いた。
これにより、CPU 版と CUDA 版でほぼ同じシーンを描画できる。

今回のランダムシーンでは、固定 4 球を含めて 43 個の球を扱った。
...
CUDA path traced spheres render:
  ...
  max_depth: 10
  total primary samples: 448000
  time: 0.330402 seconds
  primary samples/sec: 1.35592e+06
...
CPU render:
  ...
  max_depth: 10
  total primary samples: 448000
  Render time: 5.80478 seconds
  Primary samples/sec: 77177.7
```

球数が増えると、各 ray で調べる交差判定の数も増える。
そのため CPU 版はかなり遅くなったが、CUDA 版は多数のピクセルを並列に処理できるため、同じ条件でも大きく速度差が出た。

この段階で、CUDA 側でも単体の固定シーンではなく、多数の球を含む小さなランダムシーンを扱えるようになった。

## 19. コピー元の最終シーンに近い構成で CPU 版と CUDA 版を比較する

これまでの段階で、CUDA 側に次の要素がそろった。
...
そのため、コピー元の最終シーンに近い構成へ切り替えて、低サンプル数で CPU 版と CUDA 版を比較した。

今回のシーン:

- 地面: `center = (0, -1000, 0)`, `radius = 1000`
- 小球: `a = -11..10`, `b = -11..10` のグリッド候補から生成
- 中央の大球: Dielectric
- 左の大球: Lambertian
- 右の大球: Metal
...
CUDA path traced spheres render:
  ...
  focus_dist: 10
  defocus_angle: 0.6
  total primary samples: 112000
  time: 0.295318 seconds
  primary samples/sec: 379252
...
CPU render:
  ...
  max_depth: 10
  total primary samples: 112000
  Render time: 13.7079 seconds
  Primary samples/sec: 8170.45
```

低サンプルなので画像はかなりノイズが多い。
ただし、構図とシーン内容はコピー元の最終シーンに近いものになり、CPU 版と CUDA 版で近い画像が出ることを確認できた。

この条件では、CUDA 版は CPU 版より大幅に速かった。
球数が 485 個まで増えたため、CPU 側では交差判定の負荷がかなり大きくなっている。
...
- CUDA 側で CPU 版に近いカメラパラメータからレイを生成できる。
- CUDA 側で defocus blur 用にレンズ上のランダムな ray origin を扱える。
- CUDA 側で多数のランダム球を配列として扱える。
- コピー元の最終シーンに近い構成で、低サンプルの CPU / CUDA 比較ができる。

## 次にやること

コピー元の最終シーンに近い低サンプル比較まで到達した。
次は、サンプル数や解像度を少しずつ上げて、画質と実行時間の変化を見る。
